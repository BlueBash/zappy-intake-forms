...
export const useFormLogic = (config: FormConfig) => {
  ...
  const goToNext = useCallback(() => {
    ...
    if (screen.calculations) {
      const calculatedValues = performCalculations(screen.calculations, answers);
      newCalculations = { ...calculations, ...calculatedValues };
      setCalculations(newCalculations);
    }
    
    // Process eligibility rules and update flags
    const newFlags = processEligibilityRules(config.eligibility_rules, answers, newCalculations, flags);
    setFlags(newFlags);
    ...
    if (screen.next_logic) {
      ...
      nextId = evaluateLogic(screen.next_logic, currentAnswer, answers, newCalculations, newFlags);
    } 
  ...
  }, [currentScreenId, answers, calculations, screenMap, returnTo, flags, config.eligibility_rules, processEligibilityRules]);
