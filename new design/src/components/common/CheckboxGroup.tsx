...
export default function CheckboxGroup({
  ...
  // Sort options: exclusive option first, then rest
  ...
  if (exclusiveValue) {
    ...
    if (exclusiveIndex > 0) {
      ...
      sortedOptions.unshift(exclusiveOption);
    }
  }
  ...
  if (variant === 'pills') {
    return (
      <div>
        ...
        {help_text && (
          <p className="text-[13px] leading-relaxed text-[#666666] mb-4 flex items-start gap-2 md:text-sm">
          ...
          {sortedOptions.map((option, index) => {
            ...
            return (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className={`
                  pill-button relative min-h-[48px] px-5 py-3 rounded-full border-2 transition-all duration-300
                  focus:outline-none focus-visible:ring-4 focus-visible:ring-[#00A896]/20
                  ${isExclusive
                    ? isChecked
                      ? 'border-neutral-400 bg-neutral-100 text-[#2D3436] font-medium'
                      : 'border-dashed border-[#E8E8E8] bg-white text-[#666666] hover:border-neutral-400 hover:bg-neutral-50'
                    : isChecked
  ...
  return (
    <div>
      ...
      {help_text && (
        <p className="text-[13px] leading-relaxed text-neutral-600 mb-4 flex items-start gap-2 md:text-sm">
          <Info className="w-4 h-4 mt-0.5 flex-shrink-0 text-[#1a7f72]" />
          {help_text}
        </p>
      )}
        ...
        {sortedOptions.map((option, index) => {
          ...
          return (
            <motion.button
              ...
              className={`w-full py-[18px] px-5 rounded-xl border-2 transition-all duration-300 text-left group relative overflow-hidden focus:outline-none ${
                isExclusive
                  ? isChecked
                    ? 'border-gray-300 bg-white shadow-sm'
                    : 'border-dashed border-gray-300 bg-white hover:border-gray-400 hover:shadow-md shadow-sm'
                  : isChecked
                    ? 'border-[#1a7f72] bg-[#e6f3f2] shadow-md'
                    : 'border-gray-300 bg-white hover:border-[#FF7A59]/30 hover:shadow-md hover:scale-[1.02] shadow-sm'
              }`}
            ...
            >
              <div className="flex items-center justify-between gap-4">
                <span className={`leading-relaxed ${
                  isExclusive 
                    ? isChecked ? 'text-neutral-700 font-medium' : 'text-neutral-600'
                    : isChecked ? 'text-[#1a7f72] font-medium' : 'text-neutral-700'
                }`}>
                  {option.label}
                </span>
                {isChecked ? (
                  ...
                  <motion.div 
                    className={`w-6 h-6 rounded-full border-2 transition-all flex-shrink-0 ${
                      isExclusive
                        ...
                        : 'border-gray-300 group-hover:border-[#1a7f72]/50'
                    }`}
