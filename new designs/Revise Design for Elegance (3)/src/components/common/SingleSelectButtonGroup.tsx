      ...
      {options.map((option, index) => {
        ...
        return (
          <motion.button
            ...
            className={`option-button w-full py-5 sm:py-[18px] px-5 sm:px-6 rounded-xl sm:rounded-2xl border-2 transition-all duration-300 text-left group relative overflow-hidden focus:outline-none focus-visible:ring-4 focus-visible:ring-[#00A896]/20 ${
              isSelected
          ...
          >
            <div className="flex items-center justify-between gap-4">
              <span className={`text-base sm:text-lg leading-relaxed ${isSelected ? 'text-[#00A896] font-medium' : 'text-[#2D3436]'}`}>
                {option.label}
              </span>
              {isSelected ? (
                <motion.div
                ...
                <motion.div 
                  className="w-6 h-6 rounded-full border-2 border-[#E8E8E8] group-hover:border-[#00A896]/50 transition-all flex-shrink-0"
